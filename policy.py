...
class ActorCritic(nn.Module):
    ...
    def get_action(self, state, deterministic=False, exploration=0.01):
        ...
        # Store the latest state for use in entropy calculation
    ...
    def update_ac(network, rewards, log_probs, values, masks, Qval, gamma=GAMMA):
        ...
        with torch.no_grad():
            # Using the probabilities from the stored log_probs
            # First, get the action distribution by exponentiating the log_probs
            # This is a simpler approach that doesn't require the latest state
            
            # Since log_probs only contains the log prob of the selected action,
            # we'll use a different approach to estimate entropy
            
            # We can calculate the entropy directly if we have the latest state and policy distribution
            if hasattr(network, 'latest_state'):
                state = network.latest_state
                probs, _ = network.forward(state)
                # Entropy is -sum(p * log(p))
                entropy_term = -(probs * torch.log(probs + 1e-9)).sum(dim=-1).mean()
            else:
                # If we don't have the latest state, we set a small default entropy value
                # This is not ideal but prevents errors when the latest_state isn't available
                entropy_term = torch.tensor(0.1, device=device)
        ...
        # Add entropy term to the loss with a small coefficient (0.001)
        # The negative sign is because we want to maximize entropy (or minimize negative entropy)
